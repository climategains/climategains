			...
			<div v-if="user">
				<div class="mx-4 mt-14">
					...
					<div class="bg-gray/20 p-3 rounded-lg">
						<div class="border-b pb-2 mb-2">{{ user.firstname }} {{ user.lastname }}</div>
						<div>{{ user.email }}</div>
						<div>{{ user.company }}</div>
						<div>{{ user.createdAt }}</div>
					</div>
					<div v-if="store.projects && store.projects.data">
						<h1 class="mb-0 p-0 border-b pb-2 mb-3 border-white/20">My Climate Actions</h1>

						<ul class="px-1">
							<li
								class="inline-flex w-full py-3 border-b border-white/20 justify-between"
								v-for="(item, index) in store.projects.data">
								<span class="font-bold">Project Name</span><span> {{ item.name }}</span>
							</li>
						</ul>
					</div>
					<div class="button bg-blue-600 text-center py-2 my-3 font-bold rounded-full" @click="logOut()">Logout</div>
				</div>
				<ul class="hidden items m-4">
					<li>
						<svg
...
<style scoped>
.items li {
	@apply flex items-center border-b border-gray-200 py-4 text-lg;
}
</style>
