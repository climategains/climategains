	...
	<base-view>
		...
		<template #default-view-body>
			<div class="content_container">
				<div class="header_details" :style="{ backgroundImage: `url(${action.media.poster})` }">
					<h3 class="mx-4">{{ action.meta.title }}</h3>
				</div>
				...
				<div class="mx-0 mt-0 border-gray-200 leading-relaxed text-lg">
					<p class="mx-4 flex justify-between border-b border-gray-200/20 pb-3">
						<span class="font-bold">Carbon Reduction</span> {{ action.meta.funding.carbon }} kg CO2 / year
					</p>
					<p class="mx-4 flex justify-between border-b border-gray-200/20 pb-3">
						<span class="font-bold">Carbon Income</span> €{{ action.meta.funding.total }}
					</p>
					<p class="mx-4 flex justify-between border-b border-gray-200/20 pb-3">
						<span class="font-bold">Timeline</span> {{ action.meta.funding.hours }} hours
					</p>
					<p class="mx-4 flex justify-between border-b border-gray-200/20 pb-3">
						<span class="font-bold">Location</span>
						<span class="flex items-center"
							>{{ action.meta.location.town }}, {{ action.meta.location.country }}
							<svg
					...
					<p class="flex justify-between px-6">
						<span class="font-bold">Funding</span> {{ action.meta.funding.source }}
					</p>
				</div>
				...
				<div class="w-full relative flex items-center justify-center">
					<video
						class="h-auto w-full"
						controls
						src="https://2050today.org/wp-content/uploads/2020/07/Video-Placeholder.mp4?_=1" />
				</div>
				...
				<Splide :options="{ autoHeight: true }">
					<SplideSlide class="slide" v-for="(item, key) in action.media.gallery">
						<img :src="item.url" />
					</SplideSlide>
				</Splide>
				...
				<Splide :options="{ autoHeight: true }">
					<SplideSlide v-for="(item, index) in action.steps" :key="index">
						<div class="rounded-xl bg-white/10 mx-4 my-3 px-10 mb-2 py-3 flex items-center justify-center flex-col">
							<div class="rounded-2xl mt-3 mx-auto inline-flex overflow-hidden border-none">
								<span class="text-md font-bold bg-blue-800 text-white px-3 py-2">Step {{ index }}</span>
								<span class="px-3 text-md py-2 bg-gray-100/10">{{ item.hours }} Hours</span>
							</div>
							<p class="font-bold mb-3 text-xl py-2 border-b border-t border-white/20 px-2">
								<span>{{ item.title }}</span>
							</p>
							<div class="text-center px-10">
								<p class="mt-0 mb-0 block font-bold text-center underline">Materials needed</p>
								<p class="my-0">{{ item.materials }}</p>
								<p class="font-bold underline mb-0">Technical support</p>
								<p v-if="item.technical_support" class="my-0">Available</p>
				...
				<div
					class="mt-4 w-full text-center text-xl font-bold bg-blue-600 py-4 capitalize"
					@click="() => router.push(`../application/record/${action.id}`)">
					Sign up for Action
				</div>
			</div>
		</template>
