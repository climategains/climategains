...
async function getActions() {
	...
	action.value = state.actions.data.filter(x => x.id === route.params.slug)[0];
}
...
function signup() {
	if (store.user) {
		router.push(`../apply/record/${action.value.id}`);
	} else {
		modalIsOpen.value = true;
	}
}
...
<template>
	<base-view>
		...
		<template #default-view-body>
			<div v-if="state.actions && action" class="content_container mt-10">
				<div class="header_details" :style="{ backgroundImage: `url(${action.settings.media.poster.url})` }">
					<h3 class="mx-4">{{ action.name }}</h3>
				</div>
				<div class="mx-0 mt-0 border-gray-200 leading-relaxed text-lg">
					<p class="mx-4 flex justify-between border-b border-gray-200/20 pb-3">
						<span class="font-bold">Carbon Reduction</span> {{ action.emissionsAvoided }} kg CO2 / year
					</p>
					<p class="mx-4 flex justify-between border-b border-gray-200/20 pb-3">
						<span class="font-bold">Carbon Income</span> {{ action.settings.finance.fundingCurrency }}
						{{ getFunds(action.settings.project.steps) }}
					</p>
					<p class="mx-4 flex justify-between border-b border-gray-200/20 pb-3">
						<span class="font-bold">Timeline</span> {{ getHours(action.settings.project.steps) }} hours
					</p>
					<p class="mx-4 flex justify-between border-b border-gray-200/20 pb-3">
						<span class="font-bold">Location</span>
						<span class="flex items-center"
							>{{ getCountry.of(action.countryCode) }}
							<svg
					...
					<p class="flex justify-between px-6">
						<span class="font-bold">Funding</span> {{ action.settings.finance.fundingSource }}
					</p>
				</div>
				...
				<Splide :options="{ autoHeight: true }">
					<SplideSlide v-for="(item, index) in action.settings.project.steps" :key="index">
				...
				<div class="w-full flex flex-col relative flex items-center justify-center">
					<video class="h-auto w-full" controls :src="action.settings.media.video.url" />
					<p class="italic w-full my-0 bg-white/20 py-3 text-center">{{ action.settings.media.video.caption }}</p>
				</div>
				...
				<Splide :options="{ autoHeight: true }">
					<SplideSlide class="slide" v-for="(item, key) in action.settings.media.gallery">
						<img :src="item.url" />
					</SplideSlide>
				</Splide>
				<div
				...
				<ion-modal :fullscreen="true" class="" :is-open="modalIsOpen" @did-dismiss="modalIsOpen = false">
					...
					<ion-content>
						<accountView
							:callback="`../apply/record/${action.id}`"
